            ...
            for (int j = -radius; j <= radius; j++)
            {
                for (int i = -radius; i <= radius; i++)
                {
                        ...
                        neighborY >= 0 && neighborY < inputImage.height)
                    {
                        ...
                        Color pixel = (inputImage.data[neighborX + neighborY * inputImage.width];
...
Renderer::~Renderer() {
    ...
    UnloadTexture(textures[0]); // Unload texture
    UnloadTexture(textures[1]); // Unload texture
    UnloadTexture(textures[2]); // Unload texture
    UnloadModel(model);
    CloseWindow();
}
...
void Renderer::SetMask(int ind, std::string path) {
    ...
    masks[ind] = LoadTexture(path.c_str());
